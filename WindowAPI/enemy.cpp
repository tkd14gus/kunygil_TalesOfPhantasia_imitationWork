...
void golem::move()
{
	...
	case eATTACK1:
		if (_currentFrameX ==1 )
		{
			_attRcK = RectMakeCenter(_rc.left - 20, playerY, 50, playerheight);
		}
		break;
	case eATTACK2:
		if (_currentFrameX == 1)
...
void golem::draw()
{
	...
	switch (_state) {
	case eIDLE:
		_imageIdle->render(getMemDC(), _rc.left - playerWidth / 2 + 20, _rc.top - 20);
		//_imageIdle->render(getMemDC(), _rc.left - 95, _rc.top - 20);
		break;
	case eWALK:
		_imageWalk->frameRender(getMemDC(), _rc.left - playerWidth / 2 + 20, _rc.top - 20);
		//_imageWalk->frameRender(getMemDC(), _rc.left - 65, _rc.top - 20);
		break;
	case eHIT:
		_imageHit->render(getMemDC(), _rc.left - playerWidth / 2 + 20, _rc.top - 20);
		//_imageHit->render(getMemDC(), _rc.left - 75, _rc.top - 20);
		break;
	case eATTACK1:
		_imageAttack1->frameRender(getMemDC(), _rc.left - playerWidth / 2 + 20, _rc.top - 20);
		//_imageAttack1->frameRender(getMemDC(), _rc.left - 95, _rc.top - 20);
		break;
	case eATTACK2:
		_imageAttack2->frameRender(getMemDC(), _rc.left - playerWidth / 2 + 20, _rc.top - 20);
		//_imageAttack2->frameRender(getMemDC(), _rc.left - 50, _rc.top - 5);
		break;
	case eGUARD:
		_imageGuard->render(getMemDC(), _rc.left - playerWidth / 2 + 20, _rc.top - 20);
		//_imageGuard->render(getMemDC(), _rc.left - 60, _rc.top - 20);
		break;
	//case eDEAD:
