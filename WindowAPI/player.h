...
public:
	...
	void hitPlayerAttK() { _player.attXK = _player.attYK = 0; } //플레이어 공격히트했을떄 출돌사각형없애기
	void playerWin();

	tagstate getActionK() { return _state; }						//상태변경을위한 함수
	void setActionK(int pattern);				   					//상태변경 1.IDLE 2.RUN 3.HIT 4.WALK 5.JUMP 6.ATT 7.GUARD 8.DEAD 9.WIN
	void playerHit();
